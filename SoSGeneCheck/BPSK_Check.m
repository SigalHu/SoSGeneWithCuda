...
ser_sim = zeros(size(ParaList,1),length(snr_dB));
ser_theory = zeros(size(ParaList,1),length(snr_dB));
...
%             RecvSignal = awgn(LaunSignal * sqrt(RecvPower_G2) .* Nak_Lognorm_H,snr_dB(nn),'measured');
            RecvSignal = LaunSignal * sqrt(RecvPower_G2) .* Nak_Lognorm_H + wgn(1,length(LaunSignal),N0,'complex','linear');
            switch modulation
                case 'BPSK'
...
%                 grid on;
                ...
                scatterplot(RecvSignal(1:2:end));
                grid on;
            end
        end
%         if ii == 1
...
hFig = figure;
plot(snr_dB,ser_sim(1,:),SimulationColorList{1,1}, ...
     snr_dB,ser_sim(2,:),SimulationColorList{1,2}, ...
     snr_dB,ser_sim(3,:),SimulationColorList{1,3});
zoom xon;
xlabel('\itSNR\rm/dB');ylabel('ABER');
